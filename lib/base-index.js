class BaseIndex {
  constructor(opts) {
    ...
    if (opts.path) {
      this._loadFromPath(opts.path)
      return null
    }

    if (opts.docs) {
      this._addDocuments(opts.docs)
      return null
    }
  }

  _loadFromPath(path) {
    throw new Error('_loadFromPath is not implemented')
  }

  _addDocuments(documents) {
    throw new Error('_addDocuments is not implemented')
  }
