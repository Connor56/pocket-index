...
class BM25Index extends BaseIndex {
  constructor(opts) {
    ...
    const { extractor, k1 = 1.5, b = 0.75, fuzzyThreshold = 0.85 } = opts

    this._validateExtractor(extractor)

    this.extractor = extractor
    this.extractorHash = null
    this._extractorHashPromise = sha256Hex(extractor.toString())
    this.version = 1
  ...
  async load(pathOrBytes) {
    await this._ensureExtractorHash()
  ...
  async _ensureExtractorHash() {
    if (this.extractorHash === null) {
      this.extractorHash = await this._extractorHashPromise
    }
  }
  ...
  _loadFromBinary(bytes) {
    ...
    if (this.extractorHash !== state.extractorHash) {
...
async function sha256Hex(value) {
  ...
  return b4a.toString(b4a.from(digest), 'hex')
}
