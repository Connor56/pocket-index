...
class VectorIndex extends BaseIndex {
  ...
  async add(documents) {
    for (const doc of documents) {
      ...
      if (this.idSet.has(doc.id)) throw new Error(`Duplicate doc id: ${doc.id}`)
  ...
  async search(query, topK = null) {
    ...
    const distances = this.vectors.map((vec) => cosineSimilarity(normedVector, vec))
    const sortedIndices = argsort(distances, true)
    const topIds = []
    ...
    for (const index of sortedIndices) {
      ...
      topIds.push({ id: this.index[index], score: distances[index] })
    }
  ...
  _loadFromBinary(bytes) {
    const state = cenc.decode(vectorIndexCodec, bytes)
    const {
      version,
      modelId,
      dimension,
...
// Assumes the vectors come pre-normalised
